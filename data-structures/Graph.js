class Graph {
  ...
  removeVertex(vertex) {
    ...
    neighbors.from.forEach((neighbor) => {
      const vertexNeighbors = this.vertices.get(neighbor);
      vertexNeighbors.to.delete(vertex);
    });

    for (const neighbor of neighbors.to.keys()) {
      const vertexNeighbors = this.vertices.get(neighbor);
      vertexNeighbors.from.delete(vertex);
    }
